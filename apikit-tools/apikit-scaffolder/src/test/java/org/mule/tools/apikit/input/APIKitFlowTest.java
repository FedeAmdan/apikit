...
public class APIKitFlowTest {
    ...
    public void testAPIKitFlowName() {
        APIKitFlow flow = APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), RESOURCE, null, CONFIG_REF), Arrays.asList( new String[] {CONFIG_REF}));
        assertEquals(IActionType.GET.toString().toLowerCase(), flow.getAction());
    ...
    public void testAPIKitFlowNameWithContentType() {
        APIKitFlow flow = APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), RESOURCE, MIME_TYPE, CONFIG_REF), Arrays.asList( new String[] {CONFIG_REF}));
        assertEquals(IActionType.GET.toString().toLowerCase(), flow.getAction());
        assertEquals(RESOURCE, flow.getResource());
    ...
    public void testAPIKitFlowNameWithContentTypeNoConfigRef() {
        APIKitFlow flow = APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), RESOURCE, MIME_TYPE, null), Arrays.asList( new String[] {CONFIG_REF}));
        assertEquals(IActionType.GET.toString().toLowerCase(), flow.getAction());
        assertEquals(RESOURCE, flow.getResource());
        assertEquals(MIME_TYPE, flow.getMimeType());
    ...
    public void APIKitFlowNameInvalidFormat() {
        APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), null, null, null), Arrays.asList(new String[] {CONFIG_REF}));
    }

    @Test(expected = IllegalArgumentException.class)
    public void APIKitFlowNameInvalidResource() {
        APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), BAD_RESOURCE, null, CONFIG_REF), Arrays.asList( new String[] {CONFIG_REF}));
    }

    @Test
    public void APIKitFlowNameNoConfigRef() {
        APIKitFlow flow = APIKitFlow.buildFromName(buildName(IActionType.GET.toString(), RESOURCE, null, null), Arrays.asList( new String[] {CONFIG_REF}));
        assertEquals(IActionType.GET.toString().toLowerCase(), flow.getAction());
        assertEquals(RESOURCE, flow.getResource());
        assertEquals(APIKitFlow.UNNAMED_CONFIG_NAME, flow.getConfigRef());
    }
